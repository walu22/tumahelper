...
test.describe("Booking entry points", () => {
  ...
  test("hero Book help today skips old category picker", async ({ page }) => {
    await page.goto("/");
    await page.getByRole("link", { name: "Book help today" }).click();
  ...
  test("FAQ Book help now uses babysitting deep link", async ({ page }) => {
    await page.goto("/");
    await page.getByRole("link", { name: "Book help now" }).click();
    await expect(page).toHaveURL(/category=nanny.*type=babysitting/);
    await expect(page.getByRole("heading", { level: 2, name: "Booking details" })).toBeVisible({
      timeout: 15_000,
    });
  ...
  test("plain /customer/book shows service types not category cards", async ({ page }) => {
    ...
    await expect(page.getByText("Nanny Services")).toHaveCount(0);
    await expect(page.getByText("Babysitting, after-school, newborn care")).toHaveCount(0);
  });
});
