...
export function ServiceBookingSummary({
  step,
  locationAddress,
  details,
  serviceDate = "",
    ...
    <aside className="rounded-2xl border border-border bg-card overflow-hidden">
      ...
      <div className="px-5 py-4 space-y-4">
        {showEstimate && (
          <div className="rounded-xl bg-primary text-primary-foreground px-4 py-4">
            ...
            <p className="text-xs opacity-80 mt-2 leading-relaxed">
              Updates as you change your scope. Confirm at this price when you book.
            </p>
          </div>
        )}
        ...
        {hasWhere && (
          <div className="text-sm">
            <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-1">
              {details.category === "nanny" ? "Your home" : "Property"}
            </p>
        ...
        <div className="text-sm">
          ...
          <p className="font-medium text-foreground">{serviceType?.label ?? "Service"}</p>
        </div>
        ...
        {step === "scope" && scopeRows.length > 0 && (
          <div className="pt-1 border-t border-border/80">
            <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-2">
              {isAirbnb ? "Clean scope" : details.category === "nanny" ? "Care scope" : "Job scope"}
            </p>
            <ServiceScopeDetails rows={scopeRows} />
          </div>
        )}

        {step === "scope" && isAirbnb && linenLabel && (
          <div className="text-sm">
            <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-1">
              Linen
            </p>
        ...
        {step === "address" && !hasWhere && (
          <p className="text-sm text-muted-foreground flex items-start gap-2">
            ...
            Add your address to see matched{" "}
            {details.category === "nanny" ? "nannies" : "cleaners"} nearby.
          </p>
        )}
      </div>
    </aside>
