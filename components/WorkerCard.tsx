...
export function WorkerCard({ worker }: WorkerCardProps) {
  return (
    <Link href={`/workers/${worker.id}`} className="block h-full">
      <div className="bg-card rounded-lg shadow-md hover:shadow-lg transition-shadow p-4 border border-border h-full">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              <h3 className="font-semibold text-foreground truncate">{worker.full_name}</h3>
              <TrustScoreBadge
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              <span className="text-xs text-muted-foreground capitalize truncate">{worker.category.replace("_", " ")}</span>
            </div>
